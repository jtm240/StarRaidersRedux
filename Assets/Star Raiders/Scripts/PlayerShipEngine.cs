...
public class PlayerShipEngine : MonoBehaviour {
	...
	/*
	public void StartConstantThruster(float force)
	{
		thrusterForce = force;
		_isActive = true;
	}*/
	...
	{
		// moved logic here from player ship
		float forwardSpeed;
		switch (speed)
		{
		case 0:
		...
		thrusterForce = forwardSpeed * 1000;
		soundEffectVolume = 0.11f * speed;
	...
	void Update () {
		...
		if (_isActive) {
			...
			} else if (audio.volume > soundEffectVolume)
			{
				//Debug.Log ("sssh! quiet please");
				//audio.volume -= 5f * Time.deltaTime;
				audio.volume = Mathf.Clamp (audio.volume - 5f * Time.deltaTime, soundEffectVolume, audio.volume);
			}
